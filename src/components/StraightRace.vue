...
function buildStraightTrack(lanes: number) {
  ...
  const road = new THREE.Mesh(
    new THREE.PlaneGeometry(
      TRACK_LEN + 4,
      Math.max(2.4, lanes * LANE_GAP + 0.6),
    ),
  ...
  for (let li = 0; li <= lanes; li++) {
    ...
    const line = new THREE.Mesh(
      new THREE.PlaneGeometry(TRACK_LEN + 4, 0.03),
      linesMat,
    );
  ...
  const startBar = new THREE.Mesh(
    new THREE.PlaneGeometry(0.2, lanes * LANE_GAP),
    barMat,
  );
    ...
    ids.forEach((id) => {
      ...
      if (t >= 1 && !finished.has(id)) {
        ...
        if (finished.size === ids.length) {
          ...
          store.commit('FINALIZE_ROUND_RESULT', {
            round: roundCfg.value.round,
            distance: roundCfg.value.distance,
          });
...
watch(
  () => status.value,
  (nv) => {
    if (nv === 'running') startVisualRound();
  },
);
watch(
  () => roundCfg.value?.round,
  () => {
    if (status.value === 'running' && roundCfg.value) startVisualRound();
  },
);
